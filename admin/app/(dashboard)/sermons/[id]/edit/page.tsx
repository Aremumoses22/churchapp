...
import { useSermon, useUpdateSermon, useSermonSeries } from '@/hooks/useSermons';
...
export default function EditSermonPage() {
  ...
  const [form, setForm] = useState<FormState>({
    title: '', speaker: '', description: '', date: '', audioUrl: '', videoUrl: '', thumbnailUrl: '', scriptureRef: '', seriesId: '', tags: '',
  });
  ...
  if (isLoading) {
    return <div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-indigo-400" /></div>;
  }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={updateSermon.isPending} className="flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-60">
            {updateSermon.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
            Save Changes
          </button>
        </div>
      </form>
